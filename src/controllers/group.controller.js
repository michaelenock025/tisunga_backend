...
async function computeReliableSavings(groupId, storedTotalSavings) {
  ...
  return parseFloat(agg._sum.amount?.toString() ?? '0');
}

async function computeReliableBorrowed(groupId, storedTotalBorrowed) {
  const stored = parseFloat(storedTotalBorrowed?.toString() ?? '0');
  if (stored > 0) return stored;
  ...
  return parseFloat(agg._sum.principalAmount?.toString() ?? '0');
}
...
async function getMyGroup(req, res, next) {
  try {
    ...
    const totalSavings = await computeReliableSavings(membership.group.id, membership.group.totalSavings);
    const totalBorrowed = await computeReliableBorrowed(membership.group.id, membership.group.totalBorrowed);
    ...
    if (storedMySavings === 0) {
      ...
      mySavings = parseFloat(personal._sum.amount?.toString() ?? '0');
    }
    ...
    if (isLeader) {
      ...
      payload.totalBorrowed = totalBorrowed;
    }
...
async function getGroupDashboard(req, res, next) {
  try {
    ...
    const isLeader = LEADERSHIP_ROLES.includes(membership?.role);
    const totalSavings = await computeReliableSavings(groupId, group.totalSavings);
    const totalBorrowed = await computeReliableBorrowed(groupId, group.totalBorrowed);
    const availableBalance = Math.max(0, totalSavings - totalBorrowed);
    const activeLoansCount = await prisma.loan.count({ where: { groupId, status: 'ACTIVE' } });
    ...
    if (isLeader) {
      groupInfo.totalBorrowed = totalBorrowed;
      groupInfo.availableBalance = availableBalance;
    }
