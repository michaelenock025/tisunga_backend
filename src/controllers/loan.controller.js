...
async function applyForLoan(req, res, next) {
  try {
    ...
    const groupTotalSavings = parseFloat(membership.group.totalSavings.toString());
    const groupTotalBorrowed = parseFloat(membership.group.totalBorrowed.toString());
    const availableBalance = Math.max(0, groupTotalSavings - groupTotalBorrowed);
    ...
    await notifyGroupMembers({
      ...
      body: `${req.user.firstName} ${req.user.lastName} has applied for a loan of MWK ${principal.toLocaleString()}.`,
  ...
  try {
    ...
    if (payResult.status === 'FAILED') {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        await tx.group.update({ where: { id: loan.groupId }, data: { totalBorrowed: { decrement: principal } } });
      });
    ...
    await createNotification({
      ...
      body: `${req.user.firstName} ${req.user.lastName} has approved your loan of MWK ${principal.toLocaleString()}. It is being sent to your phone. Due: ${dueDate.toDateString()}.`,
      data: { loanId: loan.id },
    });
...
async function confirmRepaymentWebhook(transactionRef, status) {
  ...
  if (status === 'CONFIRMED') {
    ...
    const message = isCompleted
      ? `TISUNGA: ${borrowerName} our loan is fully repaid. Thank you!`
